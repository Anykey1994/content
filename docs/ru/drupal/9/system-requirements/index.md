...
**Системные требования** — набор необходимого для корректной работы Drupal программного обеспечения определённых версий.

Drupal разрабатывается так, что везде, где возможно, разработчику предоставляется выбор. Таким образом, вы можете выбрать, к примеру, любой тип баз данных из представленных ниже, все они поддерживаются "из коробки"; но также вы можете найти модули, добавляющие поддержку других драйверов баз данных.
...
> [!NOTE]
> Данные требования следует понимать как «ИЛИ». Вы можете использовать MySQL **или** PostgreSQL. Вам не нужно иметь всё перечисленное выше ПО, а только то, что вы желаете использовать.
...
Большая часть Drupal разработки и деплоя ведётся на Apache веб-сервере; таким образом, у сообщества больше опыта с данным веб-сервером.
...
[NGINX](http://nginx.org/) — популярный выбор в качестве веб-сервера для проектов, которые требуют высокой производительности и небольшого потребления памяти. Drupal работает как на легаси версиях NGINX: 0.7.x, 0.8.x, 1.0.x, 1.2.x), так и на стабильной 1.8.x, а также на актуальной 1.9.x.

NGINX — популярная альтернатива Apache, в связи с чем у сообщества также имеется большой опыт работы с данным веб-сервером; Drupal работает на нём без проблем.
...
Данный веб-сервер был создан для того, чтобы помочь в процессе разработки проекта. Его также можно использовать для тестирования проекта и его демонстрации. Он не разрабатывался как полноценный веб-сервер, поэтому не используйте его на продакшен серверах с публичным доступом.
